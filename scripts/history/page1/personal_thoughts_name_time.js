document.addEventListener('DOMContentLoaded', function() {
  const textInput = document.getElementById('personal_thoughts_name');
  const writtenDateInput = document.getElementById('written_date_1');
  const lastModified = document.getElementById('personal_thoughts_name_time');
  const targetElement = document.getElementById('personal_thoughts_name_history_tab'); // Change 'target_element' to the actual ID of the element you want to modify
  ...
  function updateLastModified() {
    ...
    if (!isLoggedDateEqualToLocal && enteredText !== previousInputValueOld) {
      ...
      targetElement.style.top = '240px';
      targetElement.style.left = '389.5px';
    } else {
      ...
      targetElement.style.top = '240px';
      targetElement.style.left = '433.5px';
    }
